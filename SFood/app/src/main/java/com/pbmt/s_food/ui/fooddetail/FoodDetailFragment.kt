...
import com.pbmt.s_food.ui.comment.CommentFragment
...
class FoodDetailFragment : Fragment(), TextWatcher {
    ...
    private var btn_showComment: Button?=null
    //size
    ...
    override fun onCreateView(
        ...
        initViews(root)
        foodDetailViewModel.getFoodMutableListaData().observe(viewLifecycleOwner, Observer {

            displayInfo(it)
        })
        foodDetailViewModel.getCommentMutableListaData().observe(viewLifecycleOwner,Observer{
    ...
    private fun initViews(root: View?) {
        ...
        chip_group_addon= layout_user_selected_addon.findViewById(R.id.chip_addon_display)
        ...
        btn_showComment=root.findViewById(R.id.btnShowComment)
        number_btn=root.findViewById(R.id.number_btn)
        ...
        btn_showComment!!.setOnClickListener {
    ...
    private fun displayUserSelectedAddon() {
        if (Common.foodSelected!!.userSelectedAddon != null && Common.foodSelected!!.userSelectedAddon!!.size >0){
            ...
            for (addonModel in Common.foodSelected!!.userSelectedAddon!!){
                ...
                chip.text=java.lang.StringBuilder(addonModel.name).append("(+$").append(addonModel.price).append(")").toString()
    ...
    override fun onTextChanged(chars: CharSequence?, p1: Int, p2: Int, p3: Int) {
        ...
        chip_group_addon!!.removeAllViews()
        for (addonModel in Common.foodSelected!!.addon){
            if (addonModel.name!!.toLowerCase().contains(chars.toString().toLowerCase())){
                val chip=layoutInflater.inflate(R.layout.layout_chip,null,false) as Chip
                chip.text=StringBuilder(addonModel.name!!).append("(+$").append(addonModel.price).append(")").toString()
                chip.setOnCheckedChangeListener {compoundButton,b->
                    if (b){
